...
export class AreaChartsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._irregularTimeseriesChart(
      '["--vz-primary", "--vz-warning", "--vz-success", "--vz-info"]'
    );
  }
  ...
  private generateDataSets(t: number) {
    ...
    for (let i = 0; i < 12; i++) {
      ts4 = ts4 + 86400000;
      const innerArr = [ts4, dataSeries[3][i].value];
      dataSet[3].push(innerArr);
    }
